...
ownership proof is required for either of them.
...
model Payment {
  ...
  addon       String?             // null = plan only; "whatsapp" = WhatsApp alone;
                                   // "bundle" = plan + WhatsApp in one payment
  amountPKR   Int
...
The WhatsApp channel has its own conversation cap, tracked separately from
the website plan's (`WHATSAPP_CONVERSATION_CAP` in `lib/billing/plans.ts`),
keyed off `Conversation.channel`. See section 9 for pricing and status
ladders.
...
- Do not build Slack or Instagram. Website and WhatsApp are the supported
  channels — each opt-in, self-serve, equal footing (see sections 4 and 9).
